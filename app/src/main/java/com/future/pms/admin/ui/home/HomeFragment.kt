        ...
        for (index in 0 until SLOTS.length) {
            when {
                ...
                SLOTS[index] == 'R' -> {
                    count++
                    setupParkingView(
                        count, parkingLayout, SLOTS[index], Constants.STATUS_RESERVED,
    ...
    private fun onClick(view: View) {
        if (view.tag as Int == Constants.STATUS_AVAILABLE) {
            if (Constants.selectedIds.contains(view.id.toString() + ",")) {
                Constants.selectedIds =
                    Constants.selectedIds.replace((+view.id).toString() + ",", "")
